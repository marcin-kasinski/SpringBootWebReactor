...
public class ReactiveHandler {
	...
	private static Logger log = LoggerFactory.getLogger(ReactiveHandler.class);	
...
//		String id = req.pathVariable("id");
		...
		return ServerResponse.ok().body(Mono.just(wu), WorkUnit.class);
	}
...
//		String id = req.pathVariable("id");
		...
		return ServerResponse.ok().body(Mono.just(wu).publishOn(Schedulers.elastic())
				// .subscribe(v -> System.out.prinln(""))
				.subscribeOn(Schedulers.parallel()), WorkUnit.class);
	}
...
//		String id = req.pathVariable("id");
		...
		return ServerResponse.ok().body(Mono.just(wu).publishOn(Schedulers.elastic())
				// .subscribe(v -> System.out.prinln(""))
				.subscribeOn(Schedulers.elastic()), WorkUnit.class);
	}
...
//	@Timed("mymk.metric.name")
	public Mono<ServerResponse> findAll(ServerRequest req) {
		...
		return ServerResponse.ok().body(Flux.just(wu1, wu2, wu3, wu4, wu5), WorkUnit.class);
